# TraduzAi â€” Contexto de Desenvolvimento

> Ãšltima atualizaÃ§Ã£o: 2026-04-16
> Leia este arquivo para retomar o contexto sem precisar reler o histÃ³rico de chat.

---

## O que Ã© o projeto

**TraduzAi** (antigo MangÃ¡TL) â€” App desktop para traduÃ§Ã£o automÃ¡tica de mangÃ¡/manhwa/manhua (EN â†’ PT-BR).

### Stack
- **Frontend:** React 19 + TypeScript + Tailwind CSS + Zustand (src/)
- **Backend:** Rust + Tauri v2 (src-tauri/)
- **Pipeline IA:** Python 3.12 (pipeline/)
- **ComunicaÃ§Ã£o:** Tauri IPC (invoke) + Python sidecar via stdout JSON lines

---

## O que foi feito (sessao - 2026-04-16)

### 42. Baloes conectados: limpeza sem whitening agressivo + diagonal restaurada
- **Problema:** O balao conectado estava quase no ponto ideal, mas ainda surgiam formas brancas agressivas no inpaint e, ao desligar esse whitening, os dois blocos de texto voltavam a ficar lado a lado em vez de acompanhar a diagonal natural do balao.
- **Inpaint mais limpo:** Em pipeline/vision_stack/runtime.py, o modo MANGATL_DISABLE_WHITE_BALLOON_WHITENING=1 foi refinado para desligar apenas o 	ext_box_cleanup agressivo. O cleanup de linhas e micro-residuos continua ativo, evitando quadrados brancos sem deixar sujeira escura solta.
- **Diagonal preservada no compositor:** Em pipeline/typesetter/renderer.py, os lobos conectados passaram a carregar um vies vertical por lobo. Quando o OCR ja separa bem os fragmentos, esse vies vem da geometria real detectada. Quando o renderer precisa dividir semanticamente um bloco unico em 2 lobos, ele aplica um stagger padrao para o caso left-right, mantendo a leitura na diagonal em vez de horizontalizar o par.
- **Corpo do texto preservado:** O ajuste foi feito sem desmontar a composicao aprovada do corpo do texto. O foco foi recuperar posicionamento e limpeza, nao reinventar a quebra.
- **Verificacao:** python -m unittest pipeline.tests.test_typesetting_layout pipeline.tests.test_vision_stack_runtime -> 100 testes OK, 1 skip.

### Backup
- D:/TraduzAi v0.27/ - estado com balao conectado quase final: sem whitening agressivo, cleanup macio mantido e diagonal restaurada no compositor.
- D:/TraduzAi v0.26/ - removido.

---
## O que foi feito (sessÃ£o â€” 2026-04-15)

### 41. Fix de RegressÃ£o em BalÃµes Conectados (Connected Balloons)
- **Problema:** Ao mesclar blocos de texto no renderizador, ocorria uma colisÃ£o drÃ¡stica (overlap) no 'miolo' dos balÃµes conectados e uma deformidade nas quebras de linha que esmagava o texto.
- **DeduplicaÃ§Ã£o Inteligente:** Adicionado `_dedupe_render_blocks` em `renderer.py` para ignorar a renderizaÃ§Ã£o duplicada de textos nas sub-regiÃµes secundÃ¡rias (eliminando prints fantasmas na separaÃ§Ã£o e normalizaÃ§Ã£o do texto).
- **DivisÃ£o SimÃ©trica Real baseada no OCR:** As antigas costuras (`seam_x`/`seam_y`) forÃ§avam um corte pelo centro geomÃ©trico da uniÃ£o dos balÃµes. Isso distorcia severamente os balÃµes quando um lado continha muito mais texto que o outro. Foi substituÃ­do para encontrar *estritamente o ponto mÃ©dio do espaÃ§o em branco real* entre os blocos detectados pelo OCR.
- **Abandono do Double-Expansion e Overlap Artificial:** Zerado o crescimento excessivo na interseÃ§Ã£o que cruzava os lobos e trocado por uma abordagem de Gaps. Garantido gap forte (6-8%) em conexÃµes horizontais, verticais e diagonais no `balloon_layout.py`.
- **Ajuste de Preenchimento nos Lobos:** O renderizador foi calibrado com `width_ratio` maior (0.88-0.92) para lobos para usarem perfeitamente o volume do recorte sem estourar o centro e sem necessitarem das sobreposiÃ§Ãµes fixas problemÃ¡ticas de `max_width/height` que causavam clipping vertical.
...
## O que foi feito (sessÃ£o â€” 2026-04-12)

### 38. Benchmark real do Lab elevado para >99 no capÃ­tulo 82
- `lab/benchmarking.py` passou a usar fonte efetiva no `layout_occupancy`, faixa segura para `textual_similarity`, `manual_edits_saved` filtrado por texto relevante e `visual_cleanup` alinhado ao source quando a paginaÃ§Ã£o PT Ã© incompatÃ­vel.
- `pipeline/translator/translate.py` ganhou reparo pontual com Google quando o backend local devolve vazio ou texto idÃªntico ao inglÃªs.
- `pipeline/ocr/postprocess.py` e `pipeline/vision_stack/runtime.py` passaram a cortar watermark/crÃ©ditos editoriais com mais agressividade antes de entrar no `project.json`.
- Resultado real do capÃ­tulo 82 apÃ³s rerun: `score_after 99.3`, com `textual_similarity 99.5`, `layout_occupancy 100.0`, `readability 97.0`, `visual_cleanup 99.8` e `manual_edits_saved 99.8`.

### 39. Endurecimento do filtro para `scan` / `toon` e fluxo `cntbk`
- Watermarks e crÃ©ditos com `scan`, `scans`, `scanlator`, `scanlations`, `toon`, `toons` e variantes semelhantes agora sÃ£o descartados no OCR.
- `context.md` atualizado.
- Backup versionado novo: `D:/TraduzAi v0.25/`
- Backup versionado anterior removido: `D:/TraduzAi v0.24/`

### 40. TraduÃ§Ã£o em Lote (Multi-capÃ­tulos)
- **Objetivo:** Permitir selecionar vÃ¡rios capÃ­tulos de uma vez para processamento sequencial automÃ¡tico.
...
- **Home:** Adicionado botÃ£o "TraduÃ§Ã£o em Lote" (Ã­cone Library) que permite seleÃ§Ã£o mÃºltipla.
- **Setup:** Quando em modo lote, exibe a lista de arquivos selecionados e permite remover itens. O nÃºmero do capÃ­tulo inicial Ã© definido e incrementado automaticamente para os subsequentes.
- **Processing:** RefatoraÃ§Ã£o completa para loop sequencial. Ao terminar um capÃ­tulo, o `onPipelineComplete` detecta se hÃ¡ mais itens na fila, incrementa o `batchIndex` e reinicia o pipeline para o prÃ³ximo arquivo. A UI agora mostra "Lote: X de Y" e o status dos concluÃ­dos.

### Backup
- `D:/TraduzAi v0.25/` â€” Melhoria na detecÃ§Ã£o de sub-balÃµes conectados (erosÃ£o progressiva) e distribuiÃ§Ã£o de texto multi-balÃ£o (split por sentenÃ§a/lobos).
- `D:/TraduzAi v0.24/` â€” ImplementaÃ§Ã£o completa de TraduÃ§Ã£o em Lote (Multi-capÃ­tulos). (Removido)

---

## O que foi feito (sessÃ£o â€” 2026-04-10)

### 37. ManutenÃ§Ã£o de Contexto e Backup (Fluxo `cntbk`)
- AtualizaÃ§Ã£o do arquivo `context.md` com as Ãºltimas atividades de rebranding e melhorias no editor.
- ExecuÃ§Ã£o do backup versionado do projeto (`v0.21`).
- RemoÃ§Ã£o do backup anterior (`v0.20`).

### Backup
- `D:/TraduzAi v0.23/` â€” estado atual pÃ³s-rebranding e implementaÃ§Ã£o das camadas do editor.

---

## O que foi feito (sessÃ£o â€” 2026-04-09, continuaÃ§Ã£o)

### 33. OtimizaÃ§Ã£o de Performance do OCR

**Objetivo:** Reduzir tempo de OCR por pÃ¡gina sem sacrificar qualidade.

**MudanÃ§as:**

- **`pipeline/vision_stack/runtime.py`:**
  - Removida chamada duplicada de `enrich_page_layout()` que rodava dentro de `run_detect_ocr()` E em `main.py` (economia ~100-300ms/pÃ¡g)
  - Removido import de `enrich_page_layout` (nÃ£o mais usado neste mÃ³dulo)
...
- **`pipeline/ocr/recognizer_fallback.py`:**
  - `_upscale_variant(crop)` era computado 4Ã— por regiÃ£o (redundÃ¢ncia). Agora prÃ©-computa `upscaled`, `gray`, `thresh` e `stroke_buster` uma sÃ³ vez
  - Adicionado early exit: se uma variante retorna confianÃ§a â‰¥ 0.90, pula as restantes (elimina 2-3 chamadas EasyOCR)

- **`pipeline/ocr/postprocess.py`:**
  - `_detect_italic()`: gate de tamanho aumentado de `h<14 or w<8` para `h<24 or w<16 or h*w<500` (pula HoughLinesP em regiÃµes pequenas onde itÃ¡lico nÃ£o Ã© detectÃ¡vel)
...
- **`pipeline/layout/balloon_layout.py`:**
  ...
  - Adicionado cache de subregions por chave `(inferred_bbox, balloon_bbox, tipo)` para evitar re-detecÃ§Ã£o
  - `_cached_image_bgr` Ã© removido do page_result ao final de `enrich_page_layout()` para liberar memÃ³ria
...
- **`pipeline/vision_stack/ocr.py`:**
  ...
  - `_recognize_single_paddle_with_retry()`: adicionado early exit se score â‰¥ (3, 3, 4)

**Economia estimada:** ~380-1360ms por pÃ¡gina (~10-25% de ganho no OCR)

---

### 34. OtimizaÃ§Ã£o de Performance do Pipeline Completo

**Objetivo:** Acelerar o pipeline inteiro (traduÃ§Ã£o, inpainting, typesetting, context) sem perder qualidade.

**MudanÃ§as:**

- **`pipeline/main.py` â€” TraduÃ§Ã£o + Inpainting em paralelo:**
  - TraduÃ§Ã£o (I/O-bound: chamadas HTTP) e inpainting (CPU/GPU-bound) agora rodam concorrentemente
  - TraduÃ§Ã£o roda em `ThreadPoolExecutor(max_workers=1)`, inpainting na thread principal
  - Join antes do typesetting comeÃ§ar
  - **Economia: ~15-25% do tempo total** (o estÃ¡gio mais curto dos dois fica "grÃ¡tis")

- **`pipeline/main.py` â€” Context fetch durante OCR:**
  - Fetch do AniList (atÃ© 10s de timeout) agora inicia em background durante o loop de OCR
  - Resultado Ã© coletado apÃ³s OCR terminar via `future.result(timeout=15)`
  - **Economia: ~1-10s ocultos atrÃ¡s do tempo de OCR**

- **`pipeline/translator/translate.py` â€” Google Translate batching:**
  - `translate_batch()` agora agrupa textos nÃ£o-cacheados com separador `\n\n` e traduz em chamada Ãºnica
  - Se o split do resultado nÃ£o bater, cai para fallback per-text (seguro)
  - **Economia: ~60-80% menos chamadas HTTP** (de ~100 para ~20 por capÃ­tulo)

- **`pipeline/inpainter/classical.py` â€” Feather boundary com crop local:**
  - `feather_boundary()` fazia `cv2.GaussianBlur()` na imagem inteira (ex: 1600Ã—2400)
  - Agora faz crop ao redor da mÃ¡scara, blur sÃ³ no ROI, paste back
  - **Economia: ~15-40ms por regiÃ£o**

- **`pipeline/typesetter/renderer.py` â€” Glow layer com crop local:**
  - `_apply_safe_glow()` criava `glow_layer` full-page e blurrava tudo
  - Agora computa bounding box das linhas + margem de sigmaÃ—3, cria glow_layer sÃ³ nesse tamanho
  - **Economia: ~15-40ms por bloco com glow**

- **`pipeline/typesetter/renderer.py` â€” Typesetting I/O threading:**
  - Adicionado `ThreadPoolExecutor(max_workers=1)` com prefetch da prÃ³xima imagem e save assÃ­ncrono
  - Mesmo padrÃ£o jÃ¡ usado no inpainting (`run_inpaint_pages`)
  - FreeType rendering continua serial na main thread (nÃ£o Ã© thread-safe)
  - **Economia: ~30-80ms por pÃ¡gina**

**Economia total estimada para capÃ­tulo de 20 pÃ¡ginas: ~25-40% mais rÃ¡pido**

**VerificaÃ§Ãµes:**
- `py_compile` OK para todos os arquivos modificados
- 162 testes unitÃ¡rios: mesmas falhas prÃ©-existentes (imagens de teste faltando, nomes de fonte desatualizados)
...
### Backup
- `T:/mangatl v0.19/` â€” estado antes das otimizaÃ§Ãµes de OCR
- `T:/mangatl v0.20/` â€” estado antes das otimizaÃ§Ãµes do pipeline completo

---

### 35. RemoÃ§Ã£o completa do Tesseract OCR

**Objetivo:** Remover todo o cÃ³digo morto do Tesseract OCR. O pipeline nunca usava o Tesseract no fluxo principal (os parÃ¢metros eram recebidos e imediatamente descartados com `del`). A UI de Settings mostrava seÃ§Ã£o inteira de configuraÃ§Ã£o sem efeito prÃ¡tico.

**MudanÃ§as:**

- **Frontend:**
  - `src/pages/Settings.tsx`: Removida seÃ§Ã£o inteira "OCR opcional para balÃµes limpos" (UI, estados, handlers `handleRefreshTesseract`, `handleInstallTesseract`, checkbox "OCR hÃ­brido")
  - `src/lib/tauri.ts`: Removidos `tesseract_enabled`/`tesseract_path` de `AppSettings`, removida interface `TesseractStatus`, removidas funÃ§Ãµes `checkTesseract()` e `installTesseract()`

- **Rust:**
  - `src-tauri/src/commands/settings.rs`: Removidos campos `tesseract_enabled`/`tesseract_path` de `AppSettings`, removida struct `TesseractStatus`, removidas funÃ§Ãµes `resolve_tesseract_path_from_candidates`, `discover_tesseract_path`, `check_tesseract`, `install_tesseract` (incluindo script PowerShell de instalaÃ§Ã£o via winget). Testes atualizados.
...
- **Pipeline Python:**
  ...
  - `pipeline/ocr/detector.py`: Removidos parÃ¢metros `tesseract_enabled`/`tesseract_path` de `run_ocr()` e passthrough para `run_detect_ocr`/`run_legacy_ocr`
  - `pipeline/vision_stack/runtime.py`: Removidos parÃ¢metros `tesseract_enabled`/`tesseract_path` de `run_detect_ocr()` e a linha `del` que os descartava
  - `pipeline/ocr/postprocess.py`: Removida funÃ§Ã£o `classify_region_ocr_mode()` (classificava regiÃµes como "tesseract" ou "easyocr")
  - `pipeline/ocr/reviewer.py`: Removida preferÃªncia especial por candidatos tesseract (threshold +0.02 vs +0.04)
  - `pipeline/ocr_legacy/detector.py`: Removidos parÃ¢metros tesseract, bloco `tesseract_candidates`, chamada `classify_region_ocr_mode`, import de `run_tesseract_recognition`, funÃ§Ã£o `_crop_for_mode_detection`
  - `pipeline/ocr_legacy/postprocess.py`: Removida funÃ§Ã£o `classify_region_ocr_mode()`
  - `pipeline/ocr_legacy/reviewer.py`: Removida preferÃªncia tesseract no reviewer
...
**VerificaÃ§Ãµes:** TypeScript compila OK, Rust compila OK, todos os testes Python passam.

### 36. Rebrand MangÃ¡TL â†’ TraduzAi

**MudanÃ§as observadas (feitas externamente):**
- `settings.rs`: `mangatl-pipeline.exe` â†’ `traduzai-pipeline.exe`, `mangatl-translator` â†’ `traduzai-translator`, `D:\\mangatl_data` â†’ `D:\\traduzai_data`
- `lib.rs`: Logs `[MangaTL]` â†’ `[TraduzAi]`, mensagem de erro final atualizada
- `main.py`: Docstring atualizada, `mangatl-translator` â†’ `traduzai-translator`, paths `D:/mangatl_data` â†’ `D:/traduzai_data`, campo `app` no project.json â†’ `"traduzai"`
- `Settings.tsx`: ReferÃªncias visuais atualizadas para TraduzAi, footer atualizado

---

## O que foi feito (sessÃ£o â€” 2026-04-09)

### 31. ImplementaÃ§Ã£o do Editor de Post-processamento

**Objetivo:** Permitir ediÃ§Ã£o direta e nÃ£o-destrutiva de textos, estilos e posiÃ§Ãµes pÃ³s-pipeline.

**Arquitetura do Editor:**
- **Rota `/editor`:** Interface full-screen desacoplada do layout principal.
- **`editorStore.ts`:** Estado dedicado para `pendingEdits` (buffer), zoom, pan e seleÃ§Ã£o de camadas.
- **Canvas Interativo (`EditorCanvas.tsx` + `TextOverlay.tsx`):** RenderizaÃ§Ã£o da pÃ¡gina com suporte a Zoom/Pan e overlays manipulÃ¡veis (Drag & Drop + Resize de 8 pontos).
- **Painel de Camadas (`LayersPanel.tsx`):** Lista de todos os blocos de texto com toggle de visibilidade e indicadores de status.
- **Thumbnails (`PageThumbnails.tsx`):** NavegaÃ§Ã£o lateral por pÃ¡ginas com lazy loading (Intersection Observer) para economia de memÃ³ria.
...
- **`retypeset_page` (Rust/Python):** Sistema de re-renderizaÃ§Ã£o individual de pÃ¡gina. O comando dispara o sidecar Python com a flag `--retypeset` que reconstrÃ³i a imagem traduzida em ~500ms (ignora OCR/Inpaint/Translate).
- **Auto-Update:** O editor detecta a conclusÃ£o do retypeset e recarrega os bytes da imagem traduzida via `readFile` + `URL.createObjectURL`.

### 32. RefatoraÃ§Ã£o de Rotas e Layout
- Rota `/editor` movida para fora do wrapper `<Layout>` no `App.tsx`.
- Adicionado botÃ£o "Abrir Editor" no topo do `Preview.tsx`.
- NormalizaÃ§Ã£o de caminhos de arquivo no Rust (`normalize_path`) tornada pÃºblica para uso compartilhado entre mÃ³dulos.

---

## O que foi feito (sessÃ£o â€” 2026-04-08)

### 22. Fix caminho de config_img.json: T: â†’ D:

**Arquivo:** `pipeline/teste/config_img.json`

- `source_path`, `work_dir` e `models_dir` tinham caminhos com `T:/mangatl/...` que nÃ£o existiam mais.
- Corrigidos para `d:/mangatl/...` para permitir testes locais isolados do pipeline.

---

### 23. Fix Inpainting: mÃ¡scara absorve contorno/glow/sombra do estilo detectado

**Arquivo:** `pipeline/inpainter/mask_builder.py`

**Problema:** Textos com contorno grosso, glow ou sombra deixavam "fantasmas" na imagem apÃ³s o inpainting â€” o fill cobria o miolo branco mas deixava a silhueta escura do contorno intacta.

**Fix:**
- `expand_bbox()` ganhou o parÃ¢metro `estilo: dict | None`.
- Se o dict de estilo estiver presente, extrai `contorno_px`, `glow_px` e `abs(sombra_offset[0..1])`.
- Soma esses valores Ã s margens (`margin_x`, `margin_y`), limitando a expansÃ£o extra em 18px por eixo.
...
**Problema:** EasyOCR gruadava letras com contorno grosso umas Ã s outras (ex: `WITH AN` â†’ `WIDIAN`).

**Fix:**
- Adicionada a variante `fallback-stroke-buster` ao dict de variantes do `run_fallback_recognition()`.
- FunÃ§Ã£o `_stroke_buster_variant(crop)`: aplica upscale â†’ Otsu threshold â†’ dilataÃ§Ã£o morfolÃ³gica `(3Ã—3, 1 iteraÃ§Ã£o)`, estourando o contorno agressivamente para separar letras.

---

### 25. Dados do app movidos do disco C: para D:

**Problema:** O Tauri usava `app_data_dir()` que aponta para `C:\Users\...\AppData\Roaming\com.mangatl.app\` â€” consumindo espaÃ§o no disco C.

**Fix:** Todos os comandos Rust substituÃ­ram `app.path().app_data_dir()` por `std::path::PathBuf::from("D:\\mangatl_data")`.

**Arquivos modificados:**
- `src-tauri/src/lib.rs` â€” setup inicial (cria `models/` e `projects/`)
- `src-tauri/src/commands/pipeline.rs` â€” `start_pipeline`, `warmup_visual_stack`, `check_models`, `download_models`
- `src-tauri/src/commands/settings.rs` â€” `settings_path()`, `load_settings_sync()`
- `src-tauri/src/commands/credits.rs` â€” `get_credits()`

**Nova estrutura de dados em disco:**
```
D:\mangatl_data\
  models\         â† modelos de IA (EasyOCR, corpus etc.)
  projects\       â† projetos temporÃ¡rios por job_id
  warmup\         â† logs do warmup visual
  settings.json
  credits.json
```
...
### 26. Setup.tsx: remoÃ§Ã£o da seleÃ§Ã£o de qualidade + espaÃ§amentos

**Arquivo:** `src/pages/Setup.tsx`

**MudanÃ§as:**
- Removida a UI de seleÃ§Ã£o de qualidade (botÃµes RÃ¡pida / Normal / Alta).
- Qualidade fixada como constante `DEFAULT_QUALITY = "alta"` â€” enviada sempre na navegaÃ§Ã£o para `/processing`.
- EspaÃ§amentos entre seÃ§Ãµes reduzidos (`mb-6` â†’ `mb-4`, paddings menores nos cards).
- Card de estimativa de tempo compactado: grid de 2 colunas (ritmo base + aquecimento), textos mais curtos.
- RemoÃ§Ã£o de imports nÃ£o utilizados (`formatQualityLabel`, `ProjectQuality`).

---

### 27. Pipeline sempre usa perfil mÃ¡ximo + histÃ³rico completo de pÃ¡ginas

**Arquivo:** `pipeline/main.py`

**MudanÃ§as:**
- OCR: `profile` fixado em `"max"` (antes vinha de `config.get("qualidade", "normal")`).
- RevisÃ£o contextual: `previous_pages` agora recebe **todo** o `ocr_history` acumulado (antes era `[-2:]`).
- TraduÃ§Ã£o: `qualidade` fixada em `"alta"` (mÃ¡ximo contexto e memÃ³ria lexical).

**Impacto:** todas as traduÃ§Ãµes agora usam o modo de maior precisÃ£o e contexto, independentemente da interface do usuÃ¡rio.

---

## O que foi feito (sessÃµes anteriores)

### 1. Troca de OCR: PaddleOCR â†’ EasyOCR

**Motivo:** PaddleOCR 3.x quebrou no Windows (bug oneDNN + API breaking changes).

**Arquivo:** `pipeline/ocr/detector.py` â€” reescrito do zero.

**O que faz agora:**
- EasyOCR com `easyocr.Reader(["en", "ko"])` (lazy-loaded na primeira chamada)
- PrÃ©-processamento CLAHE + upscaling 2x para imagens pequenas
...
**DependÃªncia:** `easyocr>=1.7.0`

---

### 2. Troca de inpainting: LaMA neural â†’ fill por cor de fundo

**Arquivo:** `pipeline/inpainter/lama.py` â€” reescrito.

**O que faz agora:**
- Amostra cor de fundo da borda ao redor da regiÃ£o de texto
- Se fundo uniforme (std < 45): fill simples com a cor amostrada
- Se fundo gradiente/complexo (std â‰¥ 45): OpenCV TELEA como fallback

---

### 3. Troca de tradutor: Claude Haiku API â†’ Google Translate + Ollama fallback

**Arquivo:** `pipeline/translator/translate.py` â€” reescrito.

**O que faz agora:**
- `_GoogleTranslator`: wraps `deep_translator.GoogleTranslator` com cache e retry
- `translate_batch()`: agrupa textos com separador ` ||| `
- `_postprocess()`: adaptaÃ§Ãµes PT-BR para mangÃ¡
...
**DependÃªncia:** `deep-translator>=1.11.0`
...
### 5. Extractor separado em mÃ³dulo prÃ³prio

**Arquivo criado:** `pipeline/extractor/extractor.py` + `pipeline/extractor/__init__.py`

**O que faz:**
- `extract(source_path, work_dir)` â†’ cria `work_dir/_tmp/`, extrai imagens para lÃ¡
- Suporta `.cbz`, `.zip`, pasta e imagem Ãºnica
- `cleanup(tmp_dir)` â†’ `shutil.rmtree` da pasta temp apÃ³s uso

**Fluxo de pastas (estado original):**
```
work_dir/
  _tmp/        â† extraÃ§Ã£o (temporÃ¡ria, apagada no fim)
  images/      â† imagens limpas pelo inpainting
  translated/  â† pÃ¡ginas traduzidas finais
  project.json
```
...
**Problema:** UI ficava presa em "Iniciando..." â€” Python emitia eventos antes dos listeners do frontend estarem prontos.

**Fix:**
- `Setup.tsx`: removido `startPipeline`, sÃ³ salva `qualidade` no store e navega
...
## O que foi feito (sessÃ£o de hoje â€” 2026-04-02, continuaÃ§Ã£o)

### 7. Obra (nome da obra) tornou-se opcional

**Fix em `src/pages/Setup.tsx`:**
- Removido `!project.obra` da condiÃ§Ã£o `disabled` do botÃ£o Traduzir
- Label do campo alterado para "(opcional)"

---

### 8. Fix crÃ­tico: capabilities Tauri v2 faltando (causa raiz do "Iniciando..." forever)

**Problema:** UI ficava presa em "Iniciando..." permanentemente apÃ³s clicar Traduzir.

**Causa raiz:** O arquivo `src-tauri/capabilities/default.json` nÃ£o existia. No Tauri v2, sem `core:default` nas capabilities, o `listen()` do frontend falha silenciosamente â€” os eventos `pipeline-progress` e `pipeline-complete` emitidos pelo Rust nunca chegam ao frontend.
...
- Em caso de falha, o conteÃºdo do log Ã© incluÃ­do na mensagem de erro

---

### 11. Fix Preview.tsx: imagens nÃ£o carregavam com `file:///`

**Problema:** `file:///C:/...` URLs nÃ£o funcionam no webview do Tauri v2 quando o dev server roda em `http://localhost:1420` (bloqueio cross-origin).
...
- Revoga blob URL anterior ao trocar de pÃ¡gina e no unmount

**PermissÃ£o relacionada:** `fs:allow-read-file` em `src-tauri/capabilities/default.json`
...
- Adicionado `output_path?: string` Ã  interface `Project`
- Preview e Export passaram a usar `project.output_path` (pasta de saÃ­da) em vez de `project.source_path`
...
- `Home.tsx` agora carrega `project.json`, reconstrÃ³i caminhos absolutos e popula o store antes de navegar para `/preview`
- `open_source_dialog` passou a ser usado para "Nova TraduÃ§Ã£o", permitindo arquivo ou pasta
- `load_project_json` no Rust agora aceita pasta ou caminho direto para `project.json`
- NormalizaÃ§Ã£o de caminhos `file:///` tambÃ©m foi aplicada no Rust para import/export
- `save_file_dialog` agora respeita formato selecionado:
  - `zip_full` â†’ `traduzido.zip`
  - `jpg_only` â†’ `paginas-traduzidas.zip`
  - `cbz` â†’ `traduzido.cbz`
- `export_project` agora exporta corretamente:
  - `zip_full` â†’ `translated/` + `originals/` + `project.json`
  - `jpg_only` â†’ imagens traduzidas na raiz do ZIP
  - `cbz` â†’ imagens traduzidas na raiz do arquivo CBZ

**Testes Rust adicionados em `project.rs`:**
- normalizaÃ§Ã£o de path `file:///`
- estrutura do export `zip_full`
- estrutura do export `cbz`

---

### 14. PreservaÃ§Ã£o das imagens originais

**Arquivo:** `pipeline/main.py`

**Problema:** o preview/export usava `arquivo_original`, mas a extraÃ§Ã£o original ficava em `_tmp/` e era apagada no cleanup.

**Fix:**
- Antes do cleanup, o pipeline copia as pÃ¡ginas originais para `work_dir/originals/`
...
work_dir/
  _tmp/         â† extraÃ§Ã£o temporÃ¡ria, apagada no fim
  originals/    â† pÃ¡ginas originais preservadas
  images/       â† imagens limpas pelo inpainting
  translated/   â† pÃ¡ginas finais traduzidas
  project.json
```
...
**Problema:** a UI e os commands ainda falavam em PaddleOCR/LaMA, apesar da migraÃ§Ã£o para EasyOCR + OpenCV.

**Fix:**
- `check_models()` agora verifica marker de `models/easyocr/.ready`
- inpainting passou a ser tratado como local/pronto por padrÃ£o
...
**Objetivo aprovado pelo produto:** `precisÃ£o mÃ¡xima` como diferencial, mantendo custo zero para o usuÃ¡rio final, execuÃ§Ã£o local-first e adaptaÃ§Ã£o ao hardware do PC.

**RestriÃ§Ãµes confirmadas:**
- o app precisa se adaptar bem a mÃ¡quinas com **GPU integrada**
- download inicial de modelos pode ser grande, se trouxer ganho real de precisÃ£o
- caminho para mobile existe, mas Ã© objetivo futuro
- cloud nÃ£o pode ser dependÃªncia paga

**DireÃ§Ã£o aprovada:**
- abandonar a ideia de "um OCR sÃ³" e migrar para **pipeline em camadas**
- abandonar a limpeza simples de bbox Ãºnica e migrar para **mÃ¡scara refinada + fallback progressivo**

**Arquitetura alvo (implementaÃ§Ã£o incremental):**
1. `OCR v2`
   - detector/regiÃµes
   - recognizer primÃ¡rio
   - recognizer de fallback para regiÃµes com baixa confianÃ§a
   - revisor semÃ¢ntico local para normalizar a leitura antes da traduÃ§Ã£o
2. `TraduÃ§Ã£o v2`
   - receber texto revisado, tipo do texto e contexto curto
   - reduzir erros herdados do OCR bruto
3. `Inpainting v2`
   - construir mÃ¡scara melhor por texto/balÃ£o
   - expandir bbox por heurÃ­stica
   - unir regiÃµes prÃ³ximas
   - usar caminho clÃ¡ssico em casos simples e fallback mais forte em casos difÃ­ceis
4. `Perfis por hardware`
   - `compat`: maior compatibilidade
   - `quality`: equilÃ­brio com GPU integrada
   - `max`: precisÃ£o mÃ¡xima em hardware mais forte

**Plano tÃ©cnico imediato aprovado:**
- Fase 1: refatorar o OCR atual em estÃ¡gios sem quebrar o fluxo existente
- Fase 2: adicionar fallback por confianÃ§a
- Fase 3: introduzir revisor local
- Fase 4: refinar mÃ¡scaras e inpainting progressivo
- Fase 5: selecionar comportamento por perfil de hardware

**ObservaÃ§Ã£o importante:** a implementaÃ§Ã£o deve priorizar ganhos reais de precisÃ£o mantendo compatibilidade com o pipeline atual (`project.json`, preview, export e app Tauri).
...
### 19. Regra de skip: watermarks e texto nÃ£o-inglÃªs â€” feito pelo Claude (2026-04-03)

**Objetivo:** nÃ£o fazer inpainting nem traduÃ§Ã£o em regiÃµes de marca d'Ã¡gua ou texto com caracteres nÃ£o-latinos (coreano, japonÃªs, chinÃªs, Ã¡rabe, cirÃ­lico, etc.).

**Watermarks:** jÃ¡ eram filtradas completamente no OCR desde antes â€” nÃ£o entram em `page_texts` nem em `_vision_blocks`, portanto nunca chegam ao inpainting nem Ã  traduÃ§Ã£o. Nenhuma mudanÃ§a adicional necessÃ¡ria.

**Texto nÃ£o-inglÃªs â€” arquivos modificados:**

- `pipeline/ocr/postprocess.py`:
  - Adicionado `NON_LATIN_PATTERN` (regex cobrindo Hangul, CJK, Hiragana, Katakana, Ãrabe, CirÃ­lico, DevanÃ¡gari)
  - Adicionado `is_non_english(text)`: retorna `True` se >30% dos caracteres sÃ£o nÃ£o-latinos

- `pipeline/ocr_legacy/postprocess.py`:
  - Mesmas adiÃ§Ãµes de `NON_LATIN_PATTERN` e `is_non_english(text)` (cÃ³pia independente)

- `pipeline/vision_stack/runtime.py` â€” `build_page_result()`:
...
- `pipeline/inpainter_legacy/mask_builder.py` â€” `build_mask_regions()`:
  - Pula textos com `skip_processing: True` antes de criar mÃ¡scaras de inpainting

- `pipeline/translator/translate.py`:
  - `_translate_with_google`: textos `skip_processing` recebem o original como traduÃ§Ã£o (sem chamar API)
  - `_translate_with_ollama`: textos `skip_processing` sÃ£o excluÃ­dos do payload enviado ao Ollama e recebem o original no output
  - `_passthrough`: jÃ¡ propaga o original para tudo (sem mudanÃ§a)

**Comportamento resultante:**
- Texto detectado como nÃ£o-inglÃªs mantÃ©m seu bbox no `project.json` (visÃ­vel para auditoria) mas nÃ£o Ã© inpaintado nem traduzido
- O campo `skip_processing: true` fica no dict interno de OCR; nÃ£o Ã© gravado no `project.json` final (apenas nos `ocr_results` em memÃ³ria durante o pipeline)
...
- Traduzidos via Google Translate (EN â†’ PT-BR)
- Inpainting + Typesetting concluÃ­dos
- `project.json` gerado em `pipeline/teste/output_img/`

### VerificaÃ§Ãµes de build/test
```bash
npm run build
cd src-tauri
cargo test
...
- Fluxo `/processing` â†’ `/preview` funcionando com projeto de teste

**ObservaÃ§Ã£o do teste visual:** para automaÃ§Ã£o reproduzÃ­vel, o store foi semeado via Playwright/CDP no WebView2; o bug real encontrado e corrigido nesse processo foi o escopo insuficiente do `fs:allow-read-file`

---

### 21. Font Detector (YuzuMarker ResNet50) â€” feito pelo Claude (2026-04-04)

**Arquivo novo:** `pipeline/typesetter/font_detector.py`

**O que faz:** Detecta o estilo visual da fonte no texto original do mangÃ¡ e escolhe a fonte mais parecida entre as disponÃ­veis em `fonts/`.

**Modelo:** `pk/huggingface/fffonion/yuzumarker-font-detection/yuzumarker-font-detection.safetensors`
- Arquitetura: ResNet50 (backbone), fc layer original treinada em 6162 classes CJK â€” **nÃ£o usada**
- Usa o backbone como extrator de features 2048-dim via similaridade de cosseno

**Regra de seleÃ§Ã£o:**
- `CCDaveGibbonsLower W00 Regular.ttf` em **MAIÃšSCULO** = fonte base/padrÃ£o para tudo
- Candidatas: `DK Full Blast.otf`, `SINGLE FIGHTER.otf`, `Libel Suit Suit Rg.otf`
- `Hand_Of_Sean_Demo.ttf` â€” excluÃ­da do detector por ora
- Threshold cosine similarity: **0.72** â€” abaixo disso, cai para CCDaveGibbons
- Textos com `skip_processing = True` (nÃ£o-inglÃªs) nÃ£o passam pelo detector

**IntegraÃ§Ã£o:**
- `vision_stack/runtime.py` â€” singleton `_get_font_detector()` + uso em `build_page_result()`
- `ocr_legacy/detector.py` â€” mesmo padrÃ£o
- `typesetter/renderer.py` â€” `DEFAULT_FONTS` atualizado + suporte a `force_upper` (textos em CCDaveGibbons saem em MAIÃšSCULO)
- `fonts/font-map.json` â€” atualizado com fontes reais + flag `"detector": true/false`
- `ocr/postprocess.py` e `ocr_legacy/postprocess.py` â€” helper `_find_hf_model()` para localizar modelos HuggingFace locais

**Carregamento:** lazy na primeira chamada de `detect()` â€” nÃ£o impacta startup do pipeline

---

### 20. Skill `mangatl-dev` criada â€” feito pelo Claude (2026-04-03)

**Arquivo:** `C:\Users\PICHAU\.claude\skills\mangatl-dev\SKILL.md`

**O que Ã©:** Skill unificada que serve como guia tÃ©cnico para qualquer sessÃ£o futura trabalhando no MangÃ¡TL. Cobre 4 papÃ©is em um documento:

1. **Programador/arquiteto** â€” arquitetura Tauri v2, fluxo de dados Python, contratos entre mÃ³dulos, fallbacks, padrÃµes de cÃ³digo
2. **Revisor de inpainting** â€” hierarquia de decisÃ£o (skip â†’ balÃ£o branco â†’ LaMA â†’ classical), regra do sem-blur, checklist de revisÃ£o
3. **Revisor de traduÃ§Ã£o** â€” adaptaÃ§Ãµes obrigatÃ³rias, postprocessamento, erros comuns do Google Translate
4. **Professor de PT-BR para mangÃ¡** â€” registro, contraÃ§Ãµes, exclamaÃ§Ãµes, termos que nÃ£o traduzem, erros PT-PT, tamanho de balÃ£o

**Como carregar:** em qualquer sessÃ£o nova, basta mencionar o projeto ou usar `mangatl-dev` para que o Claude carregue o skill automaticamente.
...
JPG/ZIP/CBZ/pasta
    â†“
[1] ExtraÃ§Ã£o â†’ work_dir/_tmp/
    â†“
[2] OCR â€” EasyOCR detecta texto + bbox + tipo
    â†“
[3] Contexto â€” AniList (opcional, fallback silencioso)
    â†“
[4] TraduÃ§Ã£o â€” Google Translate â†’ Ollama fallback
    â†“
[5] Inpainting â€” fill por cor de fundo â†’ OpenCV TELEA fallback â†’ work_dir/images/
    â†“
[6] PreservaÃ§Ã£o de originais â†’ work_dir/originals/
    â†“
[7] Typesetting â€” Pillow renderiza texto traduzido â†’ work_dir/translated/
    â†“
...
| CenÃ¡rio | Status |
|---------|--------|
| Pipeline isolado via terminal (JPG) | âœ… Testado e funcionando |
| Fluxo completo via UI (JPG) | âœ… Testado visualmente |
| Preview de imagens traduzidas | âœ… Testado visualmente |
| Abertura de projeto existente (pasta com `project.json`) | âœ… Corrigido |
| Export ZIP/CBZ | âœ… Coberto por teste Rust |
| CBZ/ZIP end-to-end via UI | â³ NÃ£o testado visualmente |
...
| `pipeline/typesetter/renderer.py` | Renderiza traduÃ§Ã£o na imagem com Pillow |
| `pipeline/requirements.txt` | easyocr, deep-translator, opencv, Pillow, numpy |
| `src-tauri/capabilities/default.json` | PermissÃµes Tauri v2 + escopo de leitura do plugin FS |
| `src-tauri/src/commands/pipeline.rs` | Spawna Python, lÃª stdout JSON, emite eventos Tauri, verifica EasyOCR |
| `src-tauri/src/commands/project.rs` | Dialogs, validaÃ§Ã£o de import, load de `project.json`, export ZIP/CBZ |
...
| `src/lib/stores/appStore.ts` | Estado global (projeto, pipeline, crÃ©ditos, qualidade, output_path) |
| `src/pages/Home.tsx` | Tela inicial, nova traduÃ§Ã£o, abrir projeto existente |
| `src/pages/Setup.tsx` | ConfiguraÃ§Ã£o da traduÃ§Ã£o (obra opcional, salva qualidade, navega) |
| `src/pages/Processing.tsx` | Registra listeners, inicia pipeline, carrega `project.json`, salva recentes |
| `src/pages/Preview.tsx` | VisualizaÃ§Ã£o com `readFile+blob`, navegaÃ§Ã£o, export |
| `src/pages/Settings.tsx` | Status do sistema, Ollama, EasyOCR, idioma padrÃ£o |
| `src/components/ui/Layout.tsx` | Sidebar com nav, status, botÃ£o restart |
...
| `T:/mngtl v0.02/` | Estado apÃ³s sessÃ£o anterior |
| `T:/mangatl v0.03/` | Estado apÃ³s fixes de import/export, capability FS e teste visual |
...
- **Tauri v2 capabilities sÃ£o obrigatÃ³rias** â€” sem `src-tauri/capabilities/default.json` com `core:default`, o `listen()` falha silenciosamente
- **`fs:allow-read-file` precisa de escopo real** â€” sÃ³ a permissÃ£o sem `allow` nÃ£o basta para abrir projetos salvos fora do `AppData`
- **Imagens no Preview usam `readFile + blob URL`** â€” `file:///` nÃ£o funciona no webview Tauri v2 em dev mode
- **stderr do Python vai para `pipeline.log`** em `pipeline.rs` â€” nÃ£o usar `Stdio::piped()` para esse caso
- **EasyOCR baixa modelos na primeira execuÃ§Ã£o** â€” pode demorar
- **Google Translate nÃ£o precisa de API key** â€” usa `deep-translator`
- **Ollama Ã© opcional** â€” sÃ³ entra se Google Translate falhar
- **Rust precisa recompilar** para mudanÃ§as em `src-tauri/`
- **Python nÃ£o precisa recompilar** â€” changes em `pipeline/` tÃªm efeito imediato
- **`startPipeline` Ã© chamado em `Processing.tsx`**, nÃ£o em `Setup.tsx`
...
|---------|---------------|-----|
| UI fica em "Iniciando..." para sempre | capabilities faltando ou race condition | Confirmar `core:default` no `default.json`; confirmar `startPipeline` em `Processing.tsx` apÃ³s listeners |
...
| Imagem nÃ£o carrega no Preview (tela preta) | estratÃ©gia `file:///` ou `blob` quebrada | Confirmar que `Preview.tsx` usa `readFile + blob URL` |
| Pipeline crashou silenciosamente | erro Python sem log | Ver `pipeline.log` no `work_dir` |
| EasyOCR nÃ£o detecta texto | imagem muito escura/baixo contraste | Tentar prÃ©-processar manualmente |
| Google Translate falha | rate limit | Aguardar ou usar Ollama |
| Pasta `_tmp` nÃ£o apagada | pipeline interrompido antes do cleanup | Apagar manualmente em `work_dir/_tmp` |
...
- `cd pipeline && venv/Scripts/python -m py_compile ocr/postprocess.py typesetter/renderer.py` â†’ OK
- `cd pipeline && venv/Scripts/python -m unittest discover -s tests` â†’ 22 testes, todos OK
...
### Arquivos modificados
- `pipeline/inpainter/classical.py` â€” reescrito do zero
- `pipeline/inpainter/mask_builder.py` â€” adicionada deteccao de texto vertical

### Pipeline novo (por regiao de texto)

```
classify_background()
  solid_light  â†’ balÃ¡o branco/claro (texto preto sobre branco)
  solid_dark   â†’ fundo escuro/preto
  solid_mid    â†’ cor sÃ³lida intermediÃ¡ria
  gradient     â†’ degradÃª vertical detectado
  textured     â†’ textura (linhas de acao, tramas, etc.)
       â†“
apply_fill()  â€” SEM blur nesta etapa
  solid_*    â†’ flat_fill: cor exata amostrada do anel ao redor
  gradient   â†’ gradient_fill: interpolacao vetorizada por linha
  textured   â†’ patch_copy: copia pixels reais do anel (vizinho mais proximo, chunks numpy)
       â†“
is_natural()  â†’ diferenca de cor na borda preenchida vs original
  natural     â†’ pronto
  nao natural â†’ feather_boundary(): blur Gaussiano de 2px SOMENTE na borda externa
                interior preenchido nao e tocado
```

### Regra central implementada
"Nunca borrar primeiro" â€” o blur so e aplicado apos copiar o fundo, e apenas na borda
...
- Para texto vertical: margem horizontal justa (10%), vertical minima (8%)
  â†’ mascara nao cobre area desnecessaria do balao

### Classificacao de fundo (classify_background)
- Amostra anel de 14px ao redor da bbox
- `std_br < 20` â†’ solido (claro se mean > 175, escuro se mean < 80, medio caso contrario)
- `vertical_diff > 28 e std < 55` â†’ gradiente (compara metade superior vs inferior)
- `std < 45` â†’ solido medio
- Resto â†’ texturado
...
### Verificacoes executadas
- `venv/Scripts/python -m py_compile inpainter/classical.py inpainter/mask_builder.py` â†’ OK
- `venv/Scripts/python -m unittest discover -s tests` â†’ 25 testes, todos OK
...
### Causa raiz identificada
- As linhas pretas residuais nao eram sempre "pretas absolutas"; em fundos medios/escuros elas eram apenas mais escuras que a vizinhanca, e a heuristica antiga so caÃ§ava pixels muito escuros.
...
### Causa raiz identificada
- O problema nao e um traÃ§o natural da arte e tambem nao parece ser costura de tile do `LaMa`.
...
- `[82, 1574, 714, 1693]`
- O `LaMa` reconstrÃ³i a regiao inteira, mas as bordas superior e inferior dessa faixa mascarada acabam ficando visiveis como duas linhas pretas.
...
- instancia a arquitetura YOLOv5 compatÃ­vel
...
- Esse teste valida que `text_box_cleanup`, `micro_cleanup` e `line_cleanup` nao alteram a imagem quando a regiao nao e classificada como balÃ£o branco.
...
- com `TensorRT` forÃ§ado como indisponivel apenas para esta execucao de benchmark
...
- A causa raiz para ele ter saÃ­do do fluxo nao era o modelo YuzuMarker em si.
...
- A correÃ§Ã£o mÃ­nima foi aplicada em:
- `pipeline/typesetter/renderer.py`
...
- `VocÄ™` -> `VocÃª`
- `ver atravÃ©s de todos` -> `enxergar todos`
...
- `T-THAT'S IMPOSSIBLE...` -> `I-ISSO Ã‰ IMPOSSÃVEL...`
- `COYLD THAT LIGHTBES` -> `PODERIA SER AQUELA LUZ...?!`
- `YOU SAID YOU COULD SEE THROUGH ALL MY ATTACkS, RIGHT?` -> `VocÃª disse que podia enxergar todos os meus golpes, certo?`
...
**Problema:** Segfault 0xc0000005 (Access Violation) durante typesetting, especialmente em balÃµes texturizados. Acentos (Ã§, Ã©, Ã£, Ã´, Ãº) nÃ£o renderizavam. BotÃ£o "Reiniciar app" falhava na 3Âª vez.

**Causa raiz identificada (por eliminaÃ§Ã£o):**
1. **ProcessPoolExecutor** no Windows re-executa `main.py` nos workers â†’ `BrokenProcessPool` â†’ imagens salvas sem texto
2. **matplotlib TextPath** usado para rendering/mediÃ§Ã£o criava centenas de objetos FreeType internamente, corrompendo memÃ³ria no Windows â†’ segfault
3. BalÃµes texturizados recebiam estilos com **glow/gradient/shadow** que multiplicavam chamadas TextPath (3-4x mais que balÃµes brancos)
4. `getbbox()` original chamava `_build_textpath_mask()` (TextPath) centenas de vezes durante binary search de font size
5. **PIL `ImageFont.truetype()`** tambÃ©m causa segfault com fontes de mangÃ¡ OTF/TTF no Windows
6. `app.restart()` do Tauri nÃ£o matava processos Python sidecar â†’ acÃºmulo de processos zombis

**CorreÃ§Ãµes aplicadas:**

**`pipeline/typesetter/renderer.py`:**
- `run_typesetting()` â€” sempre serial (removido ProcessPoolExecutor)
- `SafeTextPathFont.getbbox()` â€” estimativa matemÃ¡tica (`len * size * 0.55`) em vez de TextPath (zero FreeType na mediÃ§Ã£o)
- `_build_textpath_mask()` â€” substituÃ­do TextPath por `matplotlib.ft2font.FT2Font` (bitmap direto com anti-aliasing, suporta acentos)
- `render_text_block()` SafeTextPathFont branch â€” re-habilitados shadow/glow/gradient via FT2Font (seguro com poucas chamadas)
...
**`pipeline/vision_stack/runtime.py`:**
- Todos os balÃµes (brancos e texturizados) recebem mesmo tratamento de estilo
- Fonte fixa: `Newrotic.ttf` com `force_upper=True`
- Removido font detection (`fd.detect()`) para balÃµes texturizados
- Estilos (glow/gradient/shadow) vÃªm do `analyze_style()` sem override
- Inpainting continua diferenciado (branco vs texturizado)

**`fonts/font-map.json`:**
- Atualizado para refletir fontes disponÃ­veis (CCDaveGibbons, ComicNeue, Newrotic, KOMIKAX)
- Removidas referÃªncias a DK Full Blast.otf e SINGLE FIGHTER.otf (OTF problemÃ¡ticas)

**`src-tauri/src/commands/settings.rs`:**
- `restart_app()` agora mata processos `mangatl-pipeline.exe` e Python sidecar via `taskkill` antes de `app.restart()`

### RestriÃ§Ãµes tÃ©cnicas confirmadas (typesetting)
- **NÃƒO usar PIL `ImageFont.truetype()`** â€” segfault 0xc0000005 com fontes de mangÃ¡
- **NÃƒO usar ProcessPoolExecutor** â€” BrokenProcessPool no Windows
- **NÃƒO usar ThreadPoolExecutor** â€” FreeType nÃ£o Ã© thread-safe
- **NÃƒO usar matplotlib TextPath** â€” falha com acentos + acÃºmulo causa segfault
- **NÃƒO usar TextToPath singleton** â€” conflita com FT2Font separados
- **Usar FT2Font** para rendering (bitmap direto, suporta acentos)
- **Usar estimativa matemÃ¡tica** para mediÃ§Ã£o (evita centenas de chamadas FreeType)
- **ExecuÃ§Ã£o serial obrigatÃ³ria** no typesetting
- **Todos os balÃµes** recebem mesmo tratamento de estilo (sÃ³ inpainting diferencia)
- **restart_app()** deve matar sidecars antes de reiniciar

---

### Melhorias de traduÃ§Ã£o, OCR e inpainting (sessÃ£o 2026-04-08)

**Problemas resolvidos:**

#### 1. TraduÃ§Ã£o: Google Translate adicionando "CARA" e traduzindo errado em batch

**Problema:** Textos agrupados com `|||` no mesmo batch causavam contaminaÃ§Ã£o de contexto â€” o Google Translate interpretava o tom informal do mangÃ¡ e adicionava gÃ­rias como "CARA", "MANO" ou traduzia expressÃµes erradas (ex: "useless" â†’ "sem graÃ§a" em vez de "inÃºtil").

**CorreÃ§Ã£o em `pipeline/translator/translate.py`:**
- `translate_batch()` reescrito para traduzir **cada texto individualmente** (sem batch `|||`)
- Removidas constantes `BATCH_SEPARATOR` e `BATCH_MAX_CHARS` (nÃ£o mais necessÃ¡rias)
- Cache continua funcionando â€” textos repetidos nÃ£o fazem chamada extra
- `PRE_TRANSLATION_GLOSSARY` com entrada para "useless" â†’ "futile" (Google traduz "futile" corretamente como "fÃºtil/inÃºtil")
- `ADAPTATIONS`, `SOURCE_OCR_REPAIRS`, `TRANSLATION_REVIEW_REPAIRS` â€” resetados para vazio (clean slate)

#### 2. OCR: dÃ­gitos confundidos com letras em fontes estilizadas

**Problema:** PaddleOCR confunde letras com dÃ­gitos em fontes estilizadas de mangÃ¡ (ex: "I SUPPOSE" â†’ "350DDP5", "S" â†’ "5", "O" â†’ "0", "T" â†’ "7").

**CorreÃ§Ã£o em `pipeline/ocr/postprocess.py`:**
- `_DIGIT_TO_LETTER` â€” mapa de substituiÃ§Ã£o: `0â†’O, 1â†’I, 3â†’E, 4â†’A, 5â†’S, 7â†’T, 8â†’B`
- `_fix_mixed_digit_word()` â€” para cada palavra que mistura dÃ­gitos e letras, substitui dÃ­gitos pelas letras mais provÃ¡veis. NÃ£o mexe em nÃºmeros puros.
- `_remove_stray_digits()` â€” remove dÃ­gitos soltos de 1-2 caracteres no meio de texto com palavras reais (artefatos de OCR como "7" solto)
- `fix_ocr_errors()` reescrito para usar essas funÃ§Ãµes em vez de regras de regex pontuais

#### 3. Refinamento semÃ¢ntico removido

**CorreÃ§Ã£o em `pipeline/vision_stack/runtime.py`:**
...
- Texto OCR passa apenas por `fix_ocr_errors()` e vai direto para traduÃ§Ã£o

#### 4. SFX de outras lÃ­nguas: ignorados completamente

**Problema:** SFX em coreano/japonÃªs eram marcados com `skip_processing=True` mas o `_vision_block` correspondente ainda era incluÃ­do â†’ inpainting apagava o SFX original sem traduzir.

**CorreÃ§Ã£o em `pipeline/vision_stack/runtime.py`:**
- Quando `is_non_english()` retorna `True`, o bloco inteiro Ã© pulado â€” nÃ£o entra em `texts` nem em `_vision_blocks`
- Resultado: SFX em outras lÃ­nguas sÃ£o preservados na imagem original (sem inpainting, sem traduÃ§Ã£o)

#### 5. Acento "Ãš" nÃ£o renderizando (fonte Newrotic sem glyph)

**Problema:** Newrotic.ttf nÃ£o tem glyph para "Ãš" (U+00DA). FT2Font.set_text() silenciosamente pula o caractere â†’ "Ã‰ INÃšTIL." renderiza como "Ã‰ IN TIL."

**CorreÃ§Ã£o em `pipeline/typesetter/renderer.py`:**
- `_font_has_glyph(font_path, char)` â€” verifica se a fonte tem o glyph usando `get_char_index()`
- `_find_fallback_font_path(char, original_path)` â€” busca fonte fallback (ComicNeue-Bold, CCDaveGibbons) que tenha o glyph
- `_render_text_with_fallback(font, text)` â€” renderiza texto completo; quando todos os glyphs existem, renderiza de uma vez (rÃ¡pido); quando falta algum, renderiza char a char usando fallback para caracteres ausentes
...
#### 6. Inpainting pegando borda da arte em balÃµes texturizados

**Problema:** Para balÃµes texturizados, `_build_refined_bbox_mask()` expandia a mÃ¡scara com padding (12% largura, 22% altura) + dilataÃ§Ã£o, sem nenhum guard â€” a mÃ¡scara ultrapassava o bbox e apagava a borda/arte ao redor.

**CorreÃ§Ãµes em `pipeline/vision_stack/runtime.py`:**

**MÃ¡scara conservadora para texturizados:**
- Em `vision_blocks_to_mask()`, quando o balÃ£o NÃƒO Ã© branco, a mÃ¡scara refinada Ã© recortada (`clipped`) para nÃ£o ultrapassar o bbox original detectado
- Padding e dilataÃ§Ã£o do `_build_refined_bbox_mask` ainda encontram o texto, mas expansÃ£o alÃ©m do bbox Ã© removida

**RestauraÃ§Ã£o de bordas (anti-mancha-branca):**
- `_restore_textured_balloon_borders()` â€” nova funÃ§Ã£o pÃ³s-inpainting
- Para cada balÃ£o texturizado, faz blending suave com `distanceTransform`:
  - Centro da mÃ¡scara (onde estava o texto) â†’ usa resultado do inpainter
  - Bordas da mÃ¡scara â†’ transiÃ§Ã£o gradual para imagem original
- Evita manchas brancas que o inpainter deixa nas bordas de balÃµes texturizados

#### 7. BalÃµes separados sendo agrupados como um sÃ³ texto

**Problema:** `should_merge()` em `inpainter/mask_builder.py` tinha thresholds muito generosos (`horizontal_gap â‰¤ width * 0.25`, `vertical_gap â‰¤ height * 0.45`), agrupando textos de balÃµes separados numa mesma regiÃ£o. O `build_render_blocks()` no typesetter juntava esses textos num Ãºnico bloco renderizado.

**CorreÃ§Ã£o em `pipeline/inpainter/mask_builder.py`:**
- `should_merge()` â€” thresholds moderados:
  - Se bboxes se sobrepÃµem â†’ merge (mesmo balÃ£o)
  - Sem sobreposiÃ§Ã£o: `horizontal_gap â‰¤ max(8, width * 0.15)` e `vertical_gap â‰¤ max(12, height * 0.25)`
  - Antes: `0.25` e `0.45` (muito largo, juntava balÃµes separados)

**CorreÃ§Ã£o em `pipeline/layout/balloon_layout.py`:**
- `_region_supports_shared_layout()` â€” gaps reduzidos:
  - Fala: `max_vertical_gap=35, max_horizontal_gap=30` (antes 72 e 44)
  - NarraÃ§Ã£o: `max_vertical_gap=40, max_horizontal_gap=50` (antes 54 e 84)

#### 8. OCR: dÃ­gitos artefato grudados em palavras (ELE1 â†’ ELE)

**Problema:** OCR gera dÃ­gitos artefato grudados em palavras (ex: "ELE1", "1BLOQUEOU"). A correÃ§Ã£o anterior (`_fix_mixed_digit_word`) convertia `1â†’I` gerando "ELEI" (errado).

**CorreÃ§Ã£o em `pipeline/ocr/postprocess.py`:**
- `_fix_mixed_digit_word()` agora tem duas estratÃ©gias:
  - **Poucos dÃ­gitos (1-2) em palavra com letras**: remove os dÃ­gitos (`ELE1` â†’ `ELE`)
  - **Muitos dÃ­gitos misturados**: substitui por letras provÃ¡veis (`350DDP5` â†’ `ESODDS`)

#### 9. Texto ultrapassando balÃ£o texturizado

**Problema:** `width_ratio = 0.80` para balÃµes texturizados era muito largo, texto ultrapassava as bordas.

**CorreÃ§Ã£o em `pipeline/typesetter/renderer.py`:**
- `width_ratio` para balÃµes texturizados (rect) reduzido de `0.80` para `0.72`
- `padding_y` agora proporcional Ã  altura (`max(6, height * 0.10)`) em vez de fixo em 8px

### Arquivos modificados nesta sessÃ£o
- `pipeline/translator/translate.py` â€” traduÃ§Ã£o individual, glossÃ¡rios resetados
- `pipeline/ocr/postprocess.py` â€” correÃ§Ã£o genÃ©rica de OCR digit/letter, remoÃ§Ã£o de dÃ­gitos artefato
- `pipeline/vision_stack/runtime.py` â€” SFX ignorados, sem semantic refine, inpainting texturizado conservador
- `pipeline/typesetter/renderer.py` â€” fallback de fonte para glyphs ausentes, width_ratio texturizado reduzido
- `pipeline/inpainter/mask_builder.py` â€” thresholds de merge conservadores
- `pipeline/layout/balloon_layout.py` â€” shared layout mais restritivo

### Fontes em uso
| Tipo de balÃ£o | Fonte principal | Fallback (para glyphs ausentes) |
|---|---|---|
...
| `shared_layout` narraÃ§Ã£o vertical | 40px | `balloon_layout.py` |
| `shared_layout` narraÃ§Ã£o horizontal | 50px | `balloon_layout.py` |
| `width_ratio` texturizado | 0.72 | `renderer.py` |
| `width_ratio` elÃ­ptico (fala) | 0.65â€“0.75 | `renderer.py` |

---

## 2026-04-09 - Cleanup da borda do quadro vazando para dentro de balÃ£o texturizado

### Objetivo
- impedir que o inpaint copie a borda horizontal da arte para dentro de balÃµes texturizados
- corrigir o caso real da `002__002.jpg`, onde o balÃ£o vermelho estava ganhando uma faixa preta/escura interna

### Causa raiz
- no caso real, a faixa jÃ¡ nascia no `raw_output` do LaMA
- a mÃ¡scara de texto do balÃ£o vermelho ocupava um retÃ¢ngulo muito largo, e o modelo continuava a leitura do painel atrÃ¡s do balÃ£o dentro dessa Ã¡rea
- o cleanup anterior `_apply_textured_balloon_seam_cleanup(...)` removia costuras finas perto da borda da mÃ¡scara, mas nÃ£o tratava uma costura interna larga como essa

### CorreÃ§Ãµes aplicadas
- `pipeline/vision_stack/runtime.py`
- `_extract_textured_balloon_support_mask(...)` ficou mais conservadora:
- guard assimÃ©trico
- abertura morfolÃ³gica
- filtro por componente conectado ao nÃºcleo do balÃ£o
- entrou um novo pÃ³s-processo `_apply_textured_balloon_band_artifact_cleanup(...)`
- ele sÃ³ roda em balÃ£o texturizado com `balloon_bbox`
- detecta uma queda forte de luminÃ¢ncia no miolo do balÃ£o
- recompÃµe a Ã¡rea do texto com um blend vertical usando as cores originais acima/abaixo do texto
- esse passo foi ligado no fluxo real logo apÃ³s `_apply_textured_balloon_seam_cleanup(...)`
...
- recorte de inspeÃ§Ã£o:
- `D:\\mangatl\\testes\\inpaint_fix_002__002_20260409_panelborder_v3\\002__002_red_balloon_crop.jpg`
- resultado:
- a faixa preta horizontal forte deixou de atravessar o balÃ£o como antes
- ficou um blend interno suave no miolo do balÃ£o, mas sem puxar a borda do quadro para dentro

### ObservaÃ§Ã£o de validaÃ§Ã£o
- a suÃ­te completa de `test_vision_stack_runtime.py` ainda falha por motivos fora desta correÃ§Ã£o:
- fixtures locais ausentes em `D:\\mangatl\\testes\\009__001.jpg`, `010__001.jpg` e `012__001.jpg`
- falhas antigas de testes de fonte/mÃ¡scara que jÃ¡ estavam desalinhados do estado atual do projeto
---
...
- usar o mesmo caminho no processamento automÃƒÂ¡tico e no reinpaint da ediÃƒÂ§ÃƒÂ£o
- remover a tentativa de worker Rust embutido que quebrou o build do app

### Causa raiz da tentativa anterior
- o ganho do Koharu veio principalmente da estratÃƒÂ©gia `blockwise + try_fill_balloon + crop ampliado`, nÃƒÂ£o da workspace Rust inteira
...
- monta uma mÃƒÂ¡scara justa por bloco (`expand_mask=False`)
- processa bloco por bloco em janela ampliada
- tenta preencher balÃƒÂ£o simples antes de chamar o modelo
- cai para o nosso LaMA sÃƒÂ³ quando o fundo ÃƒÂ© complexo/texturizado
- costura os crops de volta na pÃƒÂ¡gina e reaplica o nosso cleanup final
- `run_inpaint_pages(...)` passou a usar esse caminho como principal, com fallback para `_apply_inpainting_round(...)` sÃƒÂ³ se o blockwise falhar
- o caminho antigo de worker Koharu foi removido do fluxo

### Limpeza do Tauri
- `src-tauri/Cargo.toml`
- removidas as dependÃƒÂªncias `koharu-*`, `camino`, `clap`, `image` e os patches de `candle/ug`
- `src-tauri/src/lib.rs`
- removido `pub mod inpaint` e o entrypoint `maybe_run_embedded_inpaint_worker()`
- `src-tauri/src/main.rs`
- removido o desvio `--inpaint-worker`
- `src-tauri/src/commands/pipeline.rs`
- `apply_sidecar_env(...)` voltou a cuidar sÃƒÂ³ de `PYTHONIOENCODING` e `PYTHONUTF8`
...
### Testes
- novas regressÃƒÂµes focadas em `pipeline/tests/test_vision_stack_runtime.py`
...
- resultado:
- o balÃƒÂ£o vermelho foi limpo sem puxar a linha horizontal do quadro para dentro
...
## 2026-04-09 - Ergonomia do editor e correï¿½ï¿½o do drag da janela
...
- isso reduz a chance de criar segmentos retos falsos no traï¿½ado do balao
- `pipeline/tests/test_vision_stack_runtime.py`
- novas regressï¿½es cobrindo arredondamento dos patches locais
...
- o repositÃ³rio/local precisa ser tratado com guardrails: o laboratorio pode preparar propostas e mudancas candidatas, mas nao deve fazer merge nem rollout sozinho
...
- `COYLD` -> `could`
- `LIGHTBES` -> `light be...?!`
- normalizacao de ruido de encoding como `VocÃ„â„¢`/`VocÄ™` -> `VocÃª`
