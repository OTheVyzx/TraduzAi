...
class TypesettingLayoutTests(unittest.TestCase):
    ...
    def test_lobe_subregion_biases_text_slightly_lower_than_geometric_center(self):
        text_data = {
            "translated": "PODE SER NADA MAIS DO QUE UM METODO DE CULTIVO INACABADO",
            "bbox": [113, 1513, 397, 1767],
            "balloon_bbox": [113, 1513, 397, 1767],
            "tipo": "fala",
        ...
        box_height = y2 - y1
        geometric_center_top = y1 + max(plan["padding_y"], (box_height - layout["total_text_height"]) // 2)

        self.assertGreater(layout["start_y"], geometric_center_top)
    ...
    def test_connected_target_sizes_reduce_overly_tall_lobe_columns(self):
        children = [
            {
                "translated": "PODE SER NADA MAIS DO QUE UM METODO DE CULTIVO INACABADO, MAS SEUS EFEITOS JA SAO MAIS DO QUE SUFICIENTES.",
                "bbox": [113, 1513, 397, 1767],
                "balloon_bbox": [113, 1513, 397, 1767],
                "tipo": "fala",
                "estilo": {"tamanho": 28, "alinhamento": "center", "fonte": "ComicNeue-Bold.ttf"},
            ...
            {
                "translated": "UM PODER QUE PERMITE SUPERAR SEUS PROPRIOS LIMITES EM UM INSTANTE.",
                "bbox": [407, 1513, 705, 1767],
                "balloon_bbox": [407, 1513, 705, 1767],
                "tipo": "fala",
                "estilo": {"tamanho": 28, "alinhamento": "center", "fonte": "ComicNeue-Bold.ttf"},
                "layout_shape": "wide",
                "layout_align": "center",
        ...
        plans = [plan_text_layout(child) for child in children]
        sizes = _resolve_connected_target_sizes(children, plans)

        self.assertEqual(len(sizes), 2)
        self.assertLessEqual(abs(sizes[0] - sizes[1]), 2)

        fixed_left = dict(plans[0])
        fixed_left["target_size"] = sizes[0]
        fixed_left["_font_search_cap"] = sizes[0]
        fixed_left["_font_search_floor"] = sizes[0]
        left_layout = _resolve_text_layout(children[0], fixed_left)

        self.assertLessEqual(len(left_layout["lines"]), 7)
        self.assertLess(left_layout["height_ratio"], 0.8)
