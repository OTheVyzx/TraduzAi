...
def resolve_windows_onnxruntime_support_dirs(package_root: str | Path | None = None) -> list[Path]:
    roots: list[Path] = []
    if package_root:
        roots.append(Path(package_root))
    else:
        ...
        if os.name == "nt":
            roots.append(python_executable.parent.parent / "Lib" / "site-packages")
        else:
            roots.extend(path for path in python_executable.parent.parent.glob("lib/python*/site-packages"))
...
def prepare_windows_onnxruntime_gpu_runtime() -> list[Path]:
    global _dll_dir_handles, _prepared_runtime_dirs

    if os.name != "nt":
        return []

    support_dirs = resolve_windows_onnxruntime_support_dirs()
    signature = tuple(str(path) for path in support_dirs)
    if signature == _prepared_runtime_dirs:
        return support_dirs
    ...
    if support_dirs:
        ...
        merged_entries = [str(path) for path in support_dirs]
        lowered = {entry.lower() for entry in merged_entries}
    ...
    _prepared_runtime_dirs = signature
    return support_dirs
...
def get_lama_session(models_dir: str | Path = "", providers: list[str] | None = None):
    global _session, _session_path, _session_providers
    prepare_windows_onnxruntime_gpu_runtime()
    ...
    if _session is None or _session_path != model_path or _session_providers != normalized_providers:
        ...
        _session_providers = normalized_providers
    return _session
