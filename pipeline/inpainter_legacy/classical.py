...
def classify_background(
    ...
    if std_br < 20:
        if mean_br > 175:
            ...
            # Guard: anel escuro pode ser a borda preta do balão, não o fundo real.
            # Se o interior do bbox for claro, a cor escura é falsa — usar interior.
            interior = _sample_interior(img_array, bbox)
            if len(interior) > 0:
                interior_br = float(np.mean(np.mean(interior, axis=1)))
                if interior_br > 150:
                    interior_color = tuple(int(c) for c in np.median(interior, axis=0))
                    return "solid_light", {"color": interior_color}
    ...
    if vertical_diff > 28 and std_br < 55:
        ...
        # Guard: se ambas as cores do gradiente forem escuras mas o interior for claro,
        # a amostragem capturou a borda do balão — usar interior como fill sólido.
        interior = _sample_interior(img_array, bbox)
        if len(interior) > 0:
            interior_br = float(np.mean(np.mean(interior, axis=1)))
            grad_br = (np.mean(top_c) + np.mean(bot_c)) / 2.0
            if grad_br < 80 and interior_br > 150:
                interior_color = tuple(int(c) for c in np.median(interior, axis=0))
                return "solid_light", {"color": interior_color}
...
def detect_white_balloon_overlay(
    ...
    # Guard: verificar que a área branca detectada COBRE OS BBOXES DE TEXTO.
    # Falsa detecção clássica: balloon_mask pega margens brancas da página ou
    # o fundo branco abaixo do balão; esses pixels ficam longe do texto e o
    # componente só passa o filtro por ter overlap marginal com inner_rect.
    # Um balão branco real deve cobrir >= 35% da área dos bboxes de texto.
    text_bbox_pixels = 0
    text_in_balloon  = 0
    for t in region.get("texts", []):
        ...
        if not tb:
            continue
        tx1, ty1, tx2, ty2 = (max(0, tb[0]), max(0, tb[1]),
                               min(width, tb[2]), min(height, tb[3]))
        text_bbox_pixels += max(0, (tx2 - tx1) * (ty2 - ty1))
        text_in_balloon  += int(np.count_nonzero(balloon_mask[ty1:ty2, tx1:tx2]))

    if text_bbox_pixels > 0 and text_in_balloon / text_bbox_pixels < 0.35:
        return None

    # Amostrar a cor do INTERIOR do balão, longe do overlay e das bordas.
    # Usar o percentil 85 dos pixels DENTRO do balloon_mask mas FORA do overlay
    # (= fundo limpo do balão, sem pixels de texto). Isso evita que o fundo
    # levemente off-white (ex: 248) arraste a mediana para baixo.
    interior_sample = img_array[(balloon_mask > 0) & (overlay_mask == 0)]
    if len(interior_sample) > 0:
        color = tuple(int(c) for c in np.percentile(interior_sample, 85, axis=0))
    else:
        pixels = img_array[balloon_mask > 0]
        if len(pixels) == 0:
            return None
        color = tuple(int(c) for c in np.percentile(pixels, 85, axis=0))
...
def detect_textured_overlay(
    ...
    if region.get("tipo") not in {"fala", "narracao"}:
        return None
    # solid_dark é incluído para capturar balões coloridos escuros (ex: vermelho, azul-escuro).
    # Para fundos realmente pretos/sem cor, _extract_textured_balloon_mask retorna None
    # (sat≈0 não passa no filtro HSV), portanto não há risco de falso positivo.
    if bg_type not in {"textured", "textured_vertical", "textured_horizontal",
                       "solid_mid", "gradient", "solid_dark"}:
        return None
    ...
    # Guard: rejeitar máscara excessivamente grande — provavelmente capturou a arte
    # do mangá ao redor, não só o balão. Limite: 2× a área do bbox da região.
    bbox_area = max(1, (bbox[2] - bbox[0]) * (bbox[3] - bbox[1]))
    if int(np.count_nonzero(balloon_mask)) > bbox_area * 2:
        return None
...
def _extract_textured_balloon_mask(
    ...
    sat = hsv[:, :, 1]
    val = hsv[:, :, 2]
    # Limita saturação mínima ao valor dominante da região para não capturar
    # toda a arte do mangá: exige que os pixels sejam ao menos tão saturados
    # quanto a mediana da borda do balão (filtra fundos acinzentados).
    dominant_sat = float(np.median(sat))
    sat_threshold = max(30, min(60, int(dominant_sat * 0.6)))
    candidate = np.where((sat > sat_threshold) & (val > 18), 255, 0).astype(np.uint8)
...
def _cardinal_seed_points(
    ...
    # Seeds externos: buscam o corpo do balão ao redor do texto.
    external = [
        (c(cx,      0, roi_w - 1), c(y1 - inset_y, 0, roi_h - 1)),
        (c(cx,      0, roi_w - 1), c(y2 + inset_y, 0, roi_h - 1)),
        (c(x1 - inset_x, 0, roi_w - 1), c(cy,      0, roi_h - 1)),
        (c(x2 + inset_x, 0, roi_w - 1), c(cy,      0, roi_h - 1)),
    ]

    # Seeds internos: usados quando bbox ≈ tamanho do balão (seeds externos
    # caem fora do balão). Amostra o fundo colorido ENTRE os caracteres.
    interior = [
        (c(cx,              0, roi_w - 1), c(cy,              0, roi_h - 1)),
        (c(cx,              0, roi_w - 1), c(y1 + h // 4,     0, roi_h - 1)),
        (c(cx,              0, roi_w - 1), c(y2 - h // 4,     0, roi_h - 1)),
        (c(x1 + w // 4,     0, roi_w - 1), c(cy,              0, roi_h - 1)),
        (c(x2 - w // 4,     0, roi_w - 1), c(cy,              0, roi_h - 1)),
    ]
...
def _sample_balloon_corner_colors(
    ...
    # Candidatos em múltiplos raios — tenta raios crescentes até achar pixels.
    # Balões circulares/ovais têm cantos fora da máscara; raio maior compensa.
    candidates = {
    ...
    for name, (px, py) in candidates.items():
        color = None
        for radius in (10, 25, 45, 70):
            color = _sample_masked_patch_color(img_array, balloon_mask, px, py, radius=radius)
    ...
    # Preencher cantos ausentes com a média dos existentes
    fallback = tuple(int(c) for c in np.mean(list(sampled.values()), axis=0))
    for name in ("nw", "ne", "sw", "se"):
        if name not in sampled:
            sampled[name] = fallback  # type: ignore[assignment]
...
def apply_fill(
    ...
    if bg_type in ("textured_vertical", "textured_horizontal"):
        # _patch_copy (nearest-neighbor do anel) é mais robusto que _directional_patch_fill
        # porque evita o artefato de "listras" quando o anel contém cores mistas.
...
def _patch_copy(
    ...
    h, w = img_array.shape[:2]
    x1, y1, x2, y2 = bbox
    ring_w = min(60, max(14, (x2 - x1) // 3, (y2 - y1) // 3))
...
def feather_boundary(
    ...
    edge_weight = (outer_edge.astype(float) / 255.0)[..., None]
    result = (img_array * (1 - edge_weight) + blurred * edge_weight)
    return result.clip(0, 255).astype(np.uint8)
...
def _sample_interior(
    ...
    """
    Retorna array (N, 3) com pixels do miolo do bbox (afastado das bordas).
    Usado para verificar se o anel externo foi contaminado pela borda do balão.
    """
    x1, y1, x2, y2 = bbox
    h, w = img_array.shape[:2]
    dx = max(4, int((x2 - x1) * shrink))
    dy = max(4, int((y2 - y1) * shrink))
    ix1 = min(w - 1, x1 + dx)
    iy1 = min(h - 1, y1 + dy)
    ix2 = max(ix1 + 1, x2 - dx)
    iy2 = max(iy1 + 1, y2 - dy)
    ...
    return img_array[iy1:iy2, ix1:ix2].reshape(-1, 3).astype(float)
