...
def run_fallback_recognition(
    ...
    # Pre-compute shared intermediates once (instead of 4x)
    upscaled = _upscale_variant(crop)
    ...
    stroke_buster = cv2.dilate(bw, kernel, iterations=1)

    variants = [
        ("fallback-upscale", upscaled),
        ("fallback-threshold", thresh),
        ("fallback-inverted", cv2.bitwise_not(thresh)),
        ("fallback-stroke-buster", stroke_buster),
    ]
    ...
    for source, variant in variants:
        ...
        # Early exit: skip remaining variants if confidence is already high
...
def _stroke_buster_variant(crop):
    """Estoura contornos grossos e glow expandindo a parte brilhante."""
    gray = cv2.cvtColor(_upscale_variant(crop), cv2.COLOR_BGR2GRAY)
    _, bw = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
