...
def _resolve_text_layout(text_data: dict, plan: dict) -> dict:
    ...
    box_width = max(1, x2 - x1)
    box_height = max(1, y2 - y1)
    vertical_bias = float(plan.get("vertical_bias", 0.0) or 0.0)
    ...
    for attempt_size in candidate_sizes:
        ...
        start_y = (
            ...
            if plan["vertical_anchor"] == "top"
                ...
                plan["padding_y"],
                (box_height - total_text_height) // 2 + int(box_height * vertical_bias),
            )
        )
    ...
    start_y = (
        ...
        if plan["vertical_anchor"] == "top"
            ...
            plan["padding_y"],
            (box_height - fallback_total_height) // 2 + int(box_height * vertical_bias),
        )
    )
    center_x = x1 + (box_width // 2)
...
def _resolve_connected_target_sizes(children: list[dict], plans: list[dict]) -> list[int]:
    ...
    for child, plan, seed_size in zip(children, plans, candidate_sizes):
        ...
        for check_size in range(seed_size, max(7, seed_size - 8), -1):
            ...
            fixed_plan["target_size"] = int(check_size)
            fixed_plan["_font_search_cap"] = int(check_size)
            fixed_plan["_font_search_floor"] = int(check_size)
            resolved_layout = _resolve_text_layout(child, fixed_plan)

            local_score = float(resolved_layout.get("score", 0.0))
            line_count = len(resolved_layout.get("lines", []))
            height_ratio = float(resolved_layout.get("height_ratio", 0.0))
            width_ratio = float(resolved_layout.get("width_ratio", 0.0))
            single_word_lines = sum(
                1 for line in resolved_layout.get("lines", []) if len(line.split()) <= 1
            )

            if child.get("_is_lobe_subregion"):
                if line_count > 6:
                    local_score -= (line_count - 6) * 2.8
                if height_ratio > 0.78:
                    local_score -= (height_ratio - 0.78) * 24.0
                elif height_ratio < 0.46:
                    local_score -= (0.46 - height_ratio) * 10.0
                if width_ratio > 0.93:
                    local_score -= (width_ratio - 0.93) * 12.0
            if single_word_lines:
                local_score -= single_word_lines * 1.6
    ...
    for combo in product(*option_groups):
        combo_sizes = [int(item["size"]) for item in combo]
        if max(combo_sizes) - min(combo_sizes) > 2:
            continue

        combo_score = sum(float(item["score"]) for item in combo)
        combo_score -= (max(combo_sizes) - min(combo_sizes)) * 0.7

        combo_line_counts = [len(item["resolved"].get("lines", [])) for item in combo]
        if len(combo_line_counts) >= 2:
            combo_score -= abs(combo_line_counts[0] - combo_line_counts[1]) * 0.5

        combo_word_counts = [len(str(child.get("translated", "") or "").split()) for child in children]
        for index in range(len(combo_sizes)):
            for other_index in range(index + 1, len(combo_sizes)):
                if combo_word_counts[index] == combo_word_counts[other_index]:
                    continue
                shorter_index = index if combo_word_counts[index] < combo_word_counts[other_index] else other_index
                longer_index = other_index if shorter_index == index else index
                if combo_sizes[shorter_index] > combo_sizes[longer_index]:
                    combo_score += (combo_sizes[shorter_index] - combo_sizes[longer_index]) * 1.0
                elif combo_sizes[shorter_index] == combo_sizes[longer_index]:
                    combo_score -= 0.35

        if combo_score > best_score:
            best_score = combo_score
            best_combo = combo_sizes

    if best_combo is not None:
        return best_combo

    quality_sizes = list(candidate_sizes)
...
def _render_single_text_block(
    ...
    if isinstance(best_font, SafeTextPathFont):
        ...
        if plan["vertical_anchor"] != "top" and positions:
            ...
            ideal_top = y1 + max(
                plan["padding_y"],
                (box_height - total_real_height) // 2 + int(box_height * float(plan.get("vertical_bias", 0.0) or 0.0)),
            )
