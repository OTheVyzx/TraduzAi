...
export TRADUZAI_WORKER_NAME="${TRADUZAI_WORKER_NAME:-vast-serverless-$(hostname)}"
...
export TRADUZAI_SERVERLESS_PORT="${TRADUZAI_SERVERLESS_PORT:-18000}"
export TRADUZAI_SERVERLESS_MODEL_LOG="${TRADUZAI_SERVERLESS_MODEL_LOG:-/tmp/traduzai-serverless-model.log}"
export PYTHONUNBUFFERED=1
export PYTHONPATH="$PROJECT_ROOT/pipeline:${PYTHONPATH:-}"

mkdir -p "$TRADUZAI_WORKER_WORK_DIR"
: > "$TRADUZAI_SERVERLESS_MODEL_LOG"

python "$PROJECT_ROOT/scripts/vast/verify-gpu-stack.py"
python -m worker.serverless_app 2>&1 | tee -a "$TRADUZAI_SERVERLESS_MODEL_LOG" &
MODEL_SERVER_PID="$!"
...
exec python "$PROJECT_ROOT/scripts/vast/serverless/worker.py"
